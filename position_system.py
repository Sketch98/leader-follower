...
class PositionSystem:
    ...
    def calc_ball_movement(self, dist_to_ball, angle_to_ball, vehicle_xy_theta, time_elapsed):
        # calculate xy dif from vehicle
        rel_ball_pos = ZERO_POS.pos_from_dist_angle(dist_to_ball, angle_to_ball)
        rel_ball_pos = rel_ball_pos.pos_from_xy_dif(Position(0, camera_y_offset))
        
        # rotate rel xy offset then add to vehicle_pos to find the ball's absolute position
        vehicle_pos, vehicle_theta = vehicle_xy_theta
        abs_ball_pos = vehicle_pos.pos_from_xy_angle_dif(rel_ball_pos, vehicle_theta)
